...
interface WeaponEditorProps {
  ...
  /**
   * Built-in field ids the active system does not use, from
   * `itemFields.hiddenBuiltIns.weapon`. Omitted means show everything, so a
  ...
  return (
    <Drawer open={open} onClose={onClose} title={weapon ? 'Edit Weapon' : 'Add Weapon'}>
      <div className="flex flex-col gap-[var(--space-md)]">
        {([['Name', 'name', 'text'], ['Range', 'range', 'text'], ['Damage', 'damage', 'text'], ['Features', 'features', 'text']] as [string, keyof typeof form, string][]).filter(([, field]) => field === 'name' || shows(field as string)).map(([label, field, type]) => (
          <div key={field}>
