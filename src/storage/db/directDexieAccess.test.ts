...
const RECORDED_DEBT: Record<string, Record<string, string>> = {
  'features/bestiary/BestiaryScreen.tsx': debt(
    'The "Add to Encounter" button on the creature detail sheet.',
    {
      'db:transaction':
        'Opens `rw` over encounters + entityLinks so the participant row and its `represents` '
        + 'edge land together or not at all.',
      'encounters:ref': SCOPE,
      'entityLinks:ref': SCOPE,
      'encounters:read':
        'Reads the encounter to append to — **and does not check `deletedAt`**. This is the '
        + 'missing line: a creature added to a soft-deleted encounter is written somewhere no '
        + 'screen lists and no Trash entry can reach.',
      'encounters:write':
        'Writes the appended participants array. It also computes `sortOrder` as '
        + '`participants.length + 1`, which collides after a mid-list removal — the sibling '
        + 'sites use `max(existing) + 1`.',
    },
  ),
  'features/encounters/CombatEncounterView.tsx': debt(
    'The quick-create-a-monster flow during combat.',
    {
      'db:transaction': 'Opens `rw` over encounters + entityLinks for the participant and its edge.',
      'encounters:ref': SCOPE,
      'entityLinks:ref': SCOPE,
      'encounters:read':
        'Reads the encounter to append to, with no `deletedAt` check — the same missing line '
        + 'as the bestiary site, reached from a different screen.',
      'encounters:write': 'Writes the appended participants array.',
    },
  ),
  'features/encounters/addPartyCharactersToEncounter.ts': debt(
    'Adds every linked party character to an encounter at once.',
    {
      'db:transaction':
        'Opens `rw` over encounters + entityLinks so the whole party and all its `represents` '
        + 'edges land in one commit rather than half a party.',
      'encounters:ref': SCOPE,
      'entityLinks:ref': SCOPE,
      'encounters:read': 'Reads the encounter to append to, with no `deletedAt` check.',
      'entityLinks:read':
        'The dedupe: an indexed `[fromEntityId+relationshipType]` lookup over the existing '
        + 'participants\' `represents` edges, so a PC already in the scene is skipped.',
      'encounters:write': 'Writes the appended participants array, once, if anything was added.',
    },
  ),
  'features/encounters/useEncounter.ts': debt(
    'The participant paths that *do* make the tombstone check — the reference implementation.',
    {
      'db:transaction':
        'Two transactions over encounters + entityLinks: adding a participant with its '
        + '`represents` edge, and removing one while soft-deleting its edges under a shared '
        + '`softDeletedBy` cascade id.',
      'encounters:ref': SCOPE,
      'entityLinks:ref': SCOPE,
      'encounters:read':
        'Reads the encounter **and refuses a tombstoned one**. This is the line the other four '
        + 'sites are missing; it moves with them so the rule has one implementation instead of '
        + 'four copies and a counterexample.',
      'entityLinks:read':
        'The PC dedupe: existing `represents` edges into the character being added, so a PC '
        + 'cannot appear twice while a creature template deliberately can.',
      'encounters:write': 'Writes the participants array.',
      'entityLinks:write':
        'Soft-deletes the removed participant\'s outgoing edges under one cascade id, so the '
        + 'removal is restorable as a unit.',
    },
  ),
