...
/** Inputs to {@link captureNpcWithNote}. */
export interface CaptureNpcInput {
  /** Session the capture happened in. */
  session: SessionTarget;
  /** Display name, used for both the creature and its note. */
  name: string;
  /** Bestiary category discriminator. */
  category: 'monster' | 'npc' | 'animal';
  /** Stat block, already keyed by the active ruleset's stat ids. */
  stats: Record<string, number>;
  /** Optional description, stored on both rows. */
  description?: string;
  /** Optional tags for the creature template. */
  tags?: string[];
  /** Encounter target, with the same three-state meaning as everywhere else. */
  targetEncounterId?: string | null;
}

/**
 * Writes a bestiary creature and the note describing it as one commit.
 *
 * @remarks
 * Four rows — template, note, `session --contains--> note`, and the encounter
 * edge when there is one — or none. A template with no note is an orphan in the
 * bestiary; a note with no template is an NPC entry whose stat block does not
 * exist. Neither is visible as a failure from the screen that wrote it.
 *
 * @returns The note and the creature-template id.
 */
export async function captureNpcWithNote(
  input: CaptureNpcInput,
): Promise<{ note: Note; creatureId: string }> {
  ...
  await db.transaction(
    ...
    async () => {
      ...
      const encounterId = await resolveEncounterAttachmentTarget({
        ...
        targetEncounterId: input.targetEncounterId,
        resolveActiveEncounterId: async (sessionId) => {
          const encounter = await encounterRepository.getActiveEncounterForSession(sessionId);
          return encounter?.id ?? null;
        },
      ...
      await persistCanonicalNoteLinks({ note, sessionId: input.session.id, encounterId });
    },
  );

  if (!written) {
    throw new Error('noteCreationService.captureNpcWithNote: the transaction wrote no note');
  }
  return { note: written, creatureId };
}
